...
void PhysfsSubsystem::find_mount_datadir()
{
    ...
    return;
#elif defined(SUPERTUX_IOS)
    if (!setup_ios_datadir())
    {
      log_warning << "Couldn't setup iOS assets: " << PHYSFS_getErrorByCode(PHYSFS_getLastErrorCode()) << std::endl;
    }
...
bool
PhysfsSubsystem::setup_ios_datadir()
{
  char* basepath_c = SDL_GetBasePath();
  std::string basepath = basepath_c ? basepath_c : "./";
  SDL_free(basepath_c);

  m_datadir = FileSystem::join(basepath, "data.zip");
  if (!FileSystem::exists(m_datadir))
  {
    log_warning << "Couldn't find bundled iOS data archive at '" << m_datadir << "'" << std::endl;
    return false;
  }

  return PHYSFS_mount(m_datadir.c_str(), nullptr, 1) != 0;
}
...
# endif
  ...
  g_config->touch_controls_visible = true;
#endif
