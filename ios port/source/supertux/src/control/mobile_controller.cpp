...
void
MobileController::update_layout(float width, float height)
{
  m_screen_width = static_cast<int>(width);
  m_screen_height = static_cast<int>(height);
...
#endif
#ifdef SUPERTUX_IOS
  const float SAFE_EDGE = 18.f;
  const float SAFE_TOP = 10.f;
#else
  const float SAFE_EDGE = 0.f;
  const float SAFE_TOP = 0.f;
#endif
  m_rect_directions.set_size(height * BUTTON_SCALE * 4 / 3, height * BUTTON_SCALE);
  m_rect_directions.set_pos(Vector(SAFE_EDGE, height - height * BUTTON_SCALE - SAFE_EDGE));
  ...
  const float joystick_zone_width = std::max(m_rect_directions.get_width(), width * 0.42f);
  const float joystick_zone_height = std::max(m_rect_directions.get_height() * 1.35f, height * 0.45f);
  m_rect_joystick_zone.set_size(joystick_zone_width, joystick_zone_height);
  m_rect_joystick_zone.set_pos(Vector(0.f, height - joystick_zone_height));

  const float button_size = height * BUTTON_SCALE;

  m_rect_jump.set_size(button_size, button_size);
  m_rect_jump.set_pos(Vector(width - button_size - SAFE_EDGE, height - button_size - SAFE_EDGE));
  m_draw_jump = m_rect_jump.grown(-m_rect_jump.get_height() * 3 / 8);

  m_rect_action.set_size(button_size, button_size);
#ifdef SUPERTUX_IOS
  m_rect_action.set_pos(Vector(m_rect_jump.get_left() - button_size * 0.62f, height - button_size - SAFE_EDGE));
#else
  m_rect_action.set_pos(Vector(width - 2 * button_size - SAFE_EDGE, height - button_size - SAFE_EDGE));
#endif
  ...
  m_rect_escape.set_pos(Vector(SAFE_EDGE + 14.f, 14.f + SAFE_TOP));
  ...
  m_rect_cheats.set_pos(Vector(width - 2 * height * BUTTON_SCALE / 2 - SAFE_EDGE, SAFE_TOP));
  ...
  m_rect_debug.set_pos(Vector(width - height * BUTTON_SCALE / 2 - SAFE_EDGE, SAFE_TOP));
  m_draw_debug = m_rect_debug.grown(-m_rect_debug.get_height() / 4);

#ifdef SUPERTUX_IOS
  m_rect_item.set_size(128.f, 128.f);
  m_rect_item.set_pos(Vector(128.f, 10.f + SAFE_TOP));
#else
  m_rect_item.set_size(128.f, 128.f);
  m_rect_item.set_pos(Vector(0.f, 0.f));
#endif
}
...
MobileController::draw(DrawingContext& context, bool movement_controls_enabled, bool editor_pause_only)
{
  ...
  if (m_screen_width != static_cast<int>(context.get_width()) ||
      ...
      m_mobile_controls_scale != g_config->m_mobile_controls_scale)
  {
    update_layout(static_cast<float>(context.get_width()),
  ...
  const float joystick_base_size = std::max(48.f, m_rect_directions.get_height() * 0.55f);
  const Rectf direction_draw_rect = g_config->touch_floating_joystick ?
    Rectf::from_center(m_joystick_center, Sizef(joystick_base_size, joystick_base_size)) :
    m_draw_directions;

  if (movement_controls_enabled && g_config->touch_floating_joystick)
  {
    const Vector fallback_center = m_rect_directions.get_middle();
    const Vector center = m_joystick_active ? m_joystick_center : fallback_center;
    const Vector pos = m_joystick_active ? m_joystick_pos : center;
    const Rectf base_rect = Rectf::from_center(center, Sizef(joystick_base_size, joystick_base_size));
    const Rectf knob_rect = Rectf::from_center(pos, Sizef(joystick_base_size * 0.42f, joystick_base_size * 0.42f));

    context.color().draw_surface_scaled(m_tex_dirs, base_rect, LAYER_GUI + 99, translucent);
    context.color().draw_surface_scaled(m_tex_btn, knob_rect, LAYER_GUI + 100, translucent);
  }
  else if (movement_controls_enabled)
  {
    context.color().draw_surface_scaled(m_tex_dirs, m_draw_directions, LAYER_GUI + 99, translucent);
  }

  if (movement_controls_enabled && m_input[CONTROL_INT(UP)])
    context.color().draw_surface_scaled(m_tex_up, direction_draw_rect, LAYER_GUI + 99, translucent);
  if (movement_controls_enabled && m_input[CONTROL_INT(DOWN)])
    context.color().draw_surface_scaled(m_tex_dwn, direction_draw_rect, LAYER_GUI + 99, translucent);
  if (movement_controls_enabled && m_input[CONTROL_INT(LEFT)])
    context.color().draw_surface_scaled(m_tex_lft, direction_draw_rect, LAYER_GUI + 99, translucent);
  if (movement_controls_enabled && m_input[CONTROL_INT(RIGHT)])
    context.color().draw_surface_scaled(m_tex_rgt, direction_draw_rect, LAYER_GUI + 99, translucent);
...
void
MobileController::update(bool movement_controls_enabled, bool editor_pause_only)
{
  if (!g_config->mobile_controls)
    return;
  ...
  {
    const Vector pos = VideoSystem::current()->get_viewport().to_logical(x, y);
    if (movement_controls_enabled &&
        g_config->touch_floating_joystick &&
        (m_mouse_joystick_active || (!m_joystick_active && m_rect_joystick_zone.contains(pos))))
    {
      if (!m_mouse_joystick_active)
      {
        m_joystick_active = true;
        m_mouse_joystick_active = true;
        m_joystick_center = pos;
      }
      m_joystick_pos = pos;
    }
    else
    {
      activate_widget_at_pos(pos.x, pos.y, movement_controls_enabled, editor_pause_only);
    }
  }
  else if (m_mouse_joystick_active)
  {
    m_mouse_joystick_active = false;
    m_joystick_active = false;
  }

  if (!movement_controls_enabled || editor_pause_only)
  {
    m_joystick_active = false;
    m_mouse_joystick_active = false;
  }

  if (movement_controls_enabled && !editor_pause_only && m_joystick_active)
    activate_floating_joystick_at_pos(m_joystick_pos);

  for (auto& i : m_fingers)
  {
    activate_widget_at_pos(i.second.x, i.second.y, movement_controls_enabled, editor_pause_only);
  }
...
MobileController::process_finger_down_event(const SDL_TouchFingerEvent& event, bool movement_controls_enabled, bool editor_pause_only)
{
  ...
  if (!editor_pause_only && movement_controls_enabled && g_config->touch_floating_joystick && m_rect_joystick_zone.contains(pos))
  {
    if (!m_joystick_active)
    {
      m_joystick_active = true;
      m_mouse_joystick_active = false;
      m_joystick_finger = event.fingerId;
      m_joystick_center = pos;
      m_joystick_pos = pos;
    }
    return true;
  ...
  const bool inside_control = contains_control_at_pos(pos, movement_controls_enabled, editor_pause_only);
  if (inside_control)
    m_fingers[event.fingerId] = pos;
  return inside_control;
}
...
MobileController::process_finger_up_event(const SDL_TouchFingerEvent& event, bool movement_controls_enabled, bool editor_pause_only)
{
  ...
  const bool was_tracked = m_fingers.erase(event.fingerId) > 0;
  return was_tracked || contains_control_at_pos(pos, movement_controls_enabled, editor_pause_only);
}
...
MobileController::process_finger_motion_event(const SDL_TouchFingerEvent& event, bool movement_controls_enabled, bool editor_pause_only)
{
  ...
  const bool was_tracked = m_fingers.find(event.fingerId) != m_fingers.end();
  const bool inside_control = contains_control_at_pos(pos, movement_controls_enabled, editor_pause_only);
  if (was_tracked || inside_control)
    m_fingers[event.fingerId] = pos;
  return was_tracked || inside_control;
}

Vector
MobileController::finger_event_to_logical(const SDL_TouchFingerEvent& event) const
{
  if (!VideoSystem::current())
    return Vector(event.x * static_cast<float>(m_screen_width),
                  event.y * static_cast<float>(m_screen_height));

  const Size window_size = VideoSystem::current()->get_window_size();
  const Viewport& viewport = VideoSystem::current()->get_viewport();
  const_cast<MobileController*>(this)->update_layout(
    static_cast<float>(viewport.get_screen_width()),
    static_cast<float>(viewport.get_screen_height()));
  return viewport.to_logical(
    static_cast<int>(event.x * static_cast<float>(window_size.width)),
    static_cast<int>(event.y * static_cast<float>(window_size.height)));
}

bool
MobileController::contains_movement_control_at_pos(const Vector& pos, bool movement_controls_enabled) const
{
  if (!movement_controls_enabled)
    return false;

  return g_config->touch_floating_joystick ?
    m_rect_joystick_zone.contains(pos) :
    m_rect_directions.contains(pos);
}

bool
MobileController::contains_control_at_pos(const Vector& pos, bool movement_controls_enabled, bool editor_pause_only) const
{
  if (editor_pause_only)
  ...
  return m_rect_jump.contains(pos) ||
    ...
    contains_movement_control_at_pos(pos, movement_controls_enabled) ||
    (g_config->developer_mode && m_rect_cheats.contains(pos)) ||
    (g_config->developer_mode && m_rect_debug.contains(pos));
}

void
MobileController::activate_widget_at_pos(float x, float y, bool movement_controls_enabled, bool editor_pause_only)
{
  if (!g_config->mobile_controls)
    return;
  ...
  else if (m_rect_action.contains(pos))
  {
  ...
  if (g_config->touch_floating_joystick)
    return;
...
void
MobileController::activate_floating_joystick_at_pos(const Vector& pos)
{
  const float deadzone = std::max(12.f, m_rect_directions.get_height() * 0.08f);
  const float max_radius = std::max(deadzone, m_rect_directions.get_height() * 0.28f);
  Vector delta = pos - m_joystick_center;
  if (glm::length(delta) > max_radius)
    delta = glm::normalize(delta) * max_radius;

  m_joystick_pos = m_joystick_center + delta;
